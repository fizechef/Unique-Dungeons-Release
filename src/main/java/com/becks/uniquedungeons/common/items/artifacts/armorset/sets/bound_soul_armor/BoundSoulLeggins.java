...
public class BoundSoulLeggins extends AbstractBoundSoulArmor implements ArmorSetItem  {
    public BoundSoulLeggins(Properties pProperties) {
        super(ArmorMaterialInit.BOUND_SOUL, EquipmentSlot.LEGS, pProperties);
    }
    ...
    public Collection<Component> generateInfoLoreText(ItemStack stack, Level level) {
        ...
        if (Screen.hasShiftDown()){
            components.add(components.size()-1,Component.translatable("lore.item.uniquedungeons.bound_soul.leggins").withStyle(ChatFormatting.GRAY));
        }
        return components;
    }
    ...
    public Collection<DamageFaktorTupel> blocking(Player player) {
        ...
        sources.add(new DamageFaktorTupel(DamageSource.LAVA, 1.0f));
        sources.add(new DamageFaktorTupel(DamageSource.IN_FIRE, 1.0f));
        sources.add(new DamageFaktorTupel(DamageSource.ON_FIRE, 1.0f));

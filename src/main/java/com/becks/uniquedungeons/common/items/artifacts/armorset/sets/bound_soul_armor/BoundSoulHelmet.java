...
public class BoundSoulHelmet  extends AbstractBoundSoulArmor implements ArmorSetItem {
    public BoundSoulHelmet(Properties pProperties) {
        super(ArmorMaterialInit.BOUND_SOUL, EquipmentSlot.HEAD, pProperties);
    }
    ...
    public Collection<Component> generateInfoLoreText(ItemStack stack, Level level) {
        ...
        if (Screen.hasShiftDown()){
            components.add(components.size()-1,Component.translatable("lore.item.uniquedungeons.bound_soul.helmet").withStyle(ChatFormatting.GRAY));
        }
        return components;
    }
    ...
    public Collection<DamageFaktorTupel> blocking(Player player) {
        ...
        sources.add(new DamageFaktorTupel(DamageSource.DROWN, 1.0f));

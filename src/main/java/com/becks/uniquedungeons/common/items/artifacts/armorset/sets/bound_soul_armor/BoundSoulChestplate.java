...
public class BoundSoulChestplate extends AbstractBoundSoulArmor implements ArmorSetItem {
    public BoundSoulChestplate(Properties pProperties) {
        super(ArmorMaterialInit.BOUND_SOUL, EquipmentSlot.CHEST, pProperties);
    }
    ...
    public Collection<Component> generateInfoLoreText(ItemStack stack, Level level) {
        ...
        if (Screen.hasShiftDown()){
            components.add(components.size()-1,Component.translatable("lore.item.uniquedungeons.bound_soul.chestplate").withStyle(ChatFormatting.GRAY));
        }
        return components;
    }
    ...
    public Collection<DamageFaktorTupel> blocking(Player player) {
        ...
        sources.add(new DamageFaktorTupel(DamageSource.MAGIC, 1.0f));
       //System.out.println(ArmorSetUtil.playerWearingFullSet(player, this.getSet()));
